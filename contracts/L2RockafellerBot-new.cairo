...
func constructor{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    amount_usdc_max: felt,
    amount_usdc_extreme: felt,
    amount_usdc_large: felt,
) {
...
@external
func send_abritrary_message{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    strategy: felt, amount: felt
) {
    ...
    send_message_to_l1(to_address=_l1_contract_address, payload_size=2, payload=message_payload);
    strategy_sent_to_l2.emit(strategy=strategy, amount=amount);
    return ();
}

@external
func calculateStrategy{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    remaining_weth: felt,
    remaining_usdc: felt,
    weth_price_ratio: felt,
    x_data_ptr_len: felt,
    x_data_ptr: felt*,
    a_num_rows: felt,
    a_num_cols: felt,
    ...
    let (msg_sender) = get_caller_address();
    assert _owner = msg_sender;
    let (weights_len, weights) = three_layer_nn(
        x_data_ptr_len,
        x_data_ptr,
        a_num_rows,
        a_num_cols,
...
func getMaxWeight{range_check_ptr}(
    weights_len: felt, weights: felt*, curr_check: felt, curr_max_index: felt
) -> (max_index: felt) {
...
func ryansFunc{syscall_ptr: felt*, range_check_ptr}() -> (weights_len: felt, weights: felt*) {
...
func compute_matvmul_by_row(
    flattened_matrix: FlattenedMatrix, vector: BasicVector, result_vector: BasicVector, row: felt
) {
    // --- End of matrix ---
...
// --- Iterating over idx ---
func _vec_add{range_check_ptr}(v1: BasicVector, v2: BasicVector, result: BasicVector, idx: felt) {
...
func linear1d_forward{range_check_ptr}(
    x: BasicVector, a: FlattenedMatrix, a_bias: BasicVector, out: BasicVector, scale_factor: felt
) {
    alloc_locals;
    ...
    local scaled_post_weights_vec: BasicVector = BasicVector(
        data_len=a.num_rows, data_ptr=scaled_post_weights
    );
    // --- Perform matvmul ---
...
func three_layer_nn{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    local a_bias: BasicVector = BasicVector(data_len=a_bias_ptr_len, data_ptr=a_bias_ptr);
    local b_matrix: FlattenedMatrix = FlattenedMatrix(
        data=b_data_ptr, num_rows=b_num_rows, num_cols=b_num_cols
    );
    local b_bias: BasicVector = BasicVector(data_len=b_bias_ptr_len, data_ptr=b_bias_ptr);
    local c_matrix: FlattenedMatrix = FlattenedMatrix(
        data=c_data_ptr, num_rows=c_num_rows, num_cols=c_num_cols
    );
    local c_bias: BasicVector = BasicVector(data_len=c_bias_ptr_len, data_ptr=c_bias_ptr);
    ...
    local x: BasicVector = BasicVector(data_len=x_data_ptr_len, data_ptr=x_data_ptr);
    local x1: BasicVector = BasicVector(data_len=a_num_rows, data_ptr=x1_data_ptr);
    local x1_relu: BasicVector = BasicVector(data_len=a_num_rows, data_ptr=x1_relu_data_ptr);
    local x2: BasicVector = BasicVector(data_len=b_num_rows, data_ptr=x2_data_ptr);
    local x2_relu: BasicVector = BasicVector(data_len=b_num_rows, data_ptr=x2_relu_data_ptr);
    local x3: BasicVector = BasicVector(data_len=c_num_rows, data_ptr=x3_data_ptr);
    // --- First layer ---
